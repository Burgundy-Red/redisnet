...
void make_package(package_t *req_package, package_t **resp_package) {
    const char * const query_head = "QUERY";
    switch (req_package->head.type) {
        case QUERY:
            *resp_package = alloc_packet((uint32_t)strlen(query_head) + 1);
            if (*resp_package) {
                memcpy((*resp_package)->data, query_head, strlen(query_head) + 1);
            } else {
                printf("alloc package error?");
            }
            break;
        case MESSAGE:
